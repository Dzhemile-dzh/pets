...
{
    /**
     * Prepares data for TrainerProfile
     *
     * @return mixed
     */
                ...
                if (!is_object($value)) {
                    foreach ($value as $key => $field_val) {
                        if (isset($field_val->group_id)) {
                            $data->statistics[$category][$key] = new \Api\Output\Mapper\TrainerProfile\StatisticsWithGroupId($field_val);
                        } else {
                            $data->statistics[$category][$key] = new \Api\Output\Mapper\TrainerProfile\StatisticsWithoutGroupId($field_val);
                        }
                    }
                }
            }
